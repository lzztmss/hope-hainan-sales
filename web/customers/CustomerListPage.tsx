  ...
  return (
    ...
    >
      ...
      <Table aria-label="客户列表">
        <TableHeader><TableRow><TableHead>客户 / 联系方式</TableHead><TableHead>营业厅 / 销售</TableHead><TableHead>客户情况</TableHead><TableHead>报价 / 订单</TableHead><TableHead>最近报价</TableHead><TableHead>操作</TableHead></TableRow></TableHeader>
        <TableBody>
        {items.map((customer) => (
          <TableRow key={customer.id}>
            <TableCell><strong>{customer.name}</strong><small>{customer.phoneMasked}</small></TableCell>
            <TableCell><strong>{customer.storeName}</strong><small>{customer.ownerName}</small></TableCell>
            <TableCell><strong>{ROOM_LABELS[customer.roomType ?? ""] ?? "户型未录入"}</strong><small>{customer.elderCount} 位长者</small></TableCell>
            <TableCell>{customer.quoteCount} / {customer.orderCount}</TableCell>
            <TableCell>{customer.lastQuoteAt ? new Date(customer.lastQuoteAt).toLocaleString("zh-CN") : "暂无"}</TableCell>
            <TableCell><Link to={`/quotes?query=${encodeURIComponent(customer.name)}`}>查看相关报价</Link></TableCell>
          </TableRow>
        ))}
        </TableBody>
      </Table>
