...
const timelineFor = (order: OrderDto): OrderTimelineEvent[] => {
  const actorFor = (...actions: readonly string[]): string => {
    const event = [...(order.lifecycleEvents ?? [])]
      .reverse()
      .find((entry) => actions.includes(entry.action));
    return event?.actorName.trim() || "历史记录未留存操作人";
  };
  const events: OrderTimelineEvent[] = [
    {
      ...
      actorName: actorFor("order.create"),
      note: "订单创建",
    },
  ];
  const append = (
    ...
    events.push({
      ...
      actorName: actorFor(...actions),
      note,
    });
  };
  append(order.acceptedAt, "accepted", "订单已受理", ["order.accept"]);
  append(order.activatedAt, "activated", "订单已生效", ["order.activate"]);
  append(order.signedAt, "signed", "订单已签收", ["order.sign"]);
  append(order.reconciledAt, "reconciled", "订单已对账", ["order.reconcile", "order.batch.reconcile"]);
  append(order.paidAt, "paid", "订单已收款", ["order.mark_paid", "order.batch.mark_paid"]);
  append(order.cancelledAt, "cancelled", "订单已取消", ["order.cancel", "order.void"]);
  return events;
};
