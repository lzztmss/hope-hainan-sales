...
const shanghaiDateInputValue = (value: Date | string): string => {
...
export const OrderDetailPage = ({
  ...
  const [error, setError] = useState<string | null>(null);
  const [busy, setBusy] = useState(false);
  const [signDateOpen, setSignDateOpen] = useState(false);
  const [actualSignedDate, setActualSignedDate] = useState(() => shanghaiDateInputValue(new Date()));
  const deleteAllowed =
    !order.deletedAt &&
  ...
  const runTransition = async (
    ...
    try {
      await onTransition?.({
        ...
        ...(signedDate ? { actualSignedDate: signedDate } : {}),
      });
      setSignDateOpen(false);
  ...
  return (
    ...
    >
      ...
      {signDateOpen ? (
        <div
          className="order-sign-dialog-backdrop"
          onClick={(event) => {
            if (event.target === event.currentTarget && !busy) setSignDateOpen(false);
          }}
        >
          <section aria-labelledby="order-sign-dialog-title" aria-modal="true" className="order-sign-dialog" role="dialog">
            <header>
              <div>
                <strong id="order-sign-dialog-title">确认实际收货日期</strong>
                <span>提成将在该日期满 7 天后进入待结算。</span>
              </div>
              <button disabled={busy} onClick={() => setSignDateOpen(false)} type="button">关闭</button>
            </header>
            <label>
              <span>实际收货日期</span>
              <input
                disabled={busy}
                max={shanghaiDateInputValue(new Date())}
                min={order.activatedAt ? shanghaiDateInputValue(order.activatedAt) : undefined}
                onChange={(event) => setActualSignedDate(event.currentTarget.value)}
                required
                type="date"
            ...
            <footer>
              <button disabled={busy} onClick={() => setSignDateOpen(false)} type="button">取消</button>
              <button
                className="order-primary-action"
                disabled={busy || !actualSignedDate}
                onClick={() => void runTransition("SIGN", actualSignedDate)}
                type="button"
              >
                {busy ? "正在确认…" : "确认签收"}
              </button>
      ...
      <footer className="order-detail-actions">
        {transitions.map((transition) => (
          <button
            ...
            onClick={() => {
              if (transition.command === "SIGN") {
                setActualSignedDate(shanghaiDateInputValue(new Date()));
                setSignDateOpen(true);
                setError(null);
                return;
              }
              void runTransition(transition.command);
            }}
            type="button"
          >
            {transition.label}
