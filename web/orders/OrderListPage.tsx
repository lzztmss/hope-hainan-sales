...
export const OrderListPage = ({
  ...
  const [batchMode, setBatchMode] = useState<"reconcile" | "receive" | null>(null);
  ...
  const payoutLabel = (order: OrderSummary): string => {
    if (order.commissionReversedFen > 0) {
      const pendingDeductionFen = Math.max(0, order.commissionPaidFen - order.commissionNetFen);
      if (pendingDeductionFen > 0) return `提成：退单待扣回 ${formatOrderMoney(pendingDeductionFen)}`;
      if (order.commissionPayoutStatus === "paid") return "提成：退单扣回已处理";
      return order.status === "partially_returned" ? "提成：部分退单后待发放" : "提成：退单后净额待发放";
    }
    return ({
      ineligible: "提成：未到发放条件",
      pending: "提成：待发放",
      paid: "提成：已发放",
    })[order.commissionPayoutStatus];
  };
    ...
    <div className="order-page">
      ...
      {detail ? (
        <div
          ...
          onClick={(event) => {
            if (event.target === event.currentTarget && !returnOpen) setDetail(null);
          }}
        >
          <OrderDetailPage
            onClose={() => {
