...
usage() {
  cat <<'EOF'
用法：upgrade.sh [--env-file FILE] [--yes]

升级前自动创建带 SHA-256 校验的一致性备份，再执行只向上的迁移。
...
printf 'PASS: 升级完成。如需应用回退，仅在数据库变更向后兼容时检出旧提交并使用对应镜像；不得自动 down-migrate。\n'
